...
bool SeedData::seedDefaultDatabase(ObjectRepository& repo) {
    ...
    auto addPlanet = [&](const std::string& slug, const std::string& name, const std::string& type,
        ...
        // State vector at perihelion
        double e = eccentricity;
        double rM = aM * (1.0 - e);
        double vPerihelion = std::sqrt((UnitConverter::G_CONST * (1.9885e30 + massKg) / aM) * ((1.0 + e) / (1.0 - e)));

        double cosA = std::cos(initialAngleRad);
        double sinA = std::sin(initialAngleRad);
        b.stateVector.positionM = glm::dvec3(rM * cosA, 0.0, rM * sinA);
        b.stateVector.velocityMps = glm::dvec3(-vPerihelion * sinA, 0.0, vPerihelion * cosA);
    ...
    addPlanet("ceres", "1 Ceres", "Dwarf Planet / Asteroid",
              9.3835e20, 469700.0, 2.767, 0.076, 0.9,
              0.09, 0.0, 4.0, 9.074,
    ...
    addPlanet("vesta", "4 Vesta", "Asteroid (Basaltic)",
              2.5908e20, 262700.0, 2.362, 0.089, 2.1,
    ...
    addPlanet("pallas", "2 Pallas", "Asteroid (Carbonaceous)",
              2.04e20, 256000.0, 2.772, 0.231, 3.8,
    ...
    addPlanet("bennu", "101955 Bennu", "Near-Earth Asteroid",
              7.329e10, 245.0, 1.126, 0.2037, 4.2,
